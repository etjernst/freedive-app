...
|---|---|---|---|---|---|
| sta-nwu-submax | NWU sub-max | hold time | session dots | up | Sub-max by design, so this trends readiness, not capacity; show RPE beside it. |
| sta-nwu-submax-moving | NWU sub-max with movement | hold time | session dots | up | Same caveat; separate history from the still variant is the point of the split. |
| sta-co2-increasing | CO2 table, increasing holds | last completed hold | rep dots | up | Until-failure: last completed hold = how far the ladder got. Total time under hold as secondary line. |
| sta-co2-vshape | CO2 table, V-shaped | total time under hold | rep dots | up | Shipped. Interrupted tables still count (the total is the point, unlike sweet-16). |
