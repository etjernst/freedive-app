...
Static is out of the Seals edition for now, so the program is the two pool sessions; the coach can add exercises later.
...
Two pool sessions a week (Jeranko adds a dry static day between them and a gym day), with a rest day before each pool session and no hard session two days running.
The Seals edition leaves strength and static work out of the app for now.
...
Ten pool exercises cover the Seals program.
...
The session-time estimate in the builder resolves percentage-of-PB targets and qualitative recoveries against the device's settings, so the coach's phone needs a template student profile (DYNB PB, DYNB pace in seconds per 25 m, breath length) to time these sessions before any member has entered a number.
Every Seals exercise is absolute distances and rests, or a percentage of the DYNB PB, and times cleanly.
