...
Third, distances and rests are set for a template student who swims DYNB around 75 m and holds around 3:00 STA, and every distance is a knob the coach can turn down to 25 m.
...
Two pool sessions and one dry static session a week, with a rest day before each pool session and no hard session two days running.
...
| Dry | static | CO2 table | warm-up ladder + submax, CO2 table | odd weeks warm-up ladder + submax; even weeks FRC O2 table |
...
Thirteen exercises cover the program.
Nine already exist in the canonical library and only need Seals defaults and phase tags; four are new.
...
|---|---|---|---|---|---|---|
| Fixed-rest set | DYNB | co2 | base, specialization, taper | 4 x 50 m, rest cap 60 s, dive pace | 4 x 50 m DNF, cap 45 s | dyn-volume-fixed |
| Pyramid | DYNB | co2 | base | 50-25-25-50, rest ≤ 60 s | 75-50-25-50-75 DYN, rest as short as possible | dyn-pyramid |
| Hypercapnic surface swim | any (freestyle, short fins) | co2 | all four | 100 m, 3 breaths per 25 m | 200 m, 2–3 breaths per 25 m | dyn-hypercapnic |
...
| Sprints | DYNB | fitness_lactic | base, build | 2 x 6 x 25 m, 20 s between, 2 min between sets | 3 x 8 x 25 m, 15 s, 2 min | dyn-sprints |
...
| Submax dive | DYNB | o2_hypoxia, mental | build, specialization, taper | 65% of PB, no upper limit, no packing | 65% then 70–80% then 80–90% with packing | dyn-nwu-submax |
...
Dry: CO2 table, 10 min, 4 breaths between holds, holds kept comfortable, meaning to the first strong contractions and no further.
...
Dry: warm-up ladder plus one submax, then after 10 min of tidal breathing the CO2 table at 3 breaths between holds.
...
Dry: odd weeks warm-up ladder plus one submax; even weeks the FRC O2 table, four holds at 60% in week 10 and 70% in week 12.
...
The session-time estimate in the builder resolves percentage-of-PB targets and qualitative recoveries against the device's settings, so the coach's phone needs a template student profile (DYNB PB, STA PB, DYNB pace in seconds per 25 m, breath length) to time these sessions before any member has entered a number.
Two of the exercises above have terminations the estimator handles only with a planning input: the CO2 table is duration-capped (fine, the cap is the time), and the warm-up ladder's first two holds are contraction-relative, which needs a first-contraction baseline in the profile (say 1:30 for the template student).
Everything else is absolute distances and rests and times cleanly.

## Open choices for Emilia and the coach

The template student's numbers: DYNB 75 m and STA 3:00 are placeholders.
Whether the fixed-rest set defaults to 50 m or 25 m for the club; 50 m assumes a 25 m pool and members who swim 75 m plus.
Whether the FRC O2 table stays in a club edition at all, or is tagged for coach-led sessions only.
