...
catalog = [
    ...
    (
        "dyn-volume-maximize",
        "Volume: maximize total distance",
        "any",
        "main",
        "volume, o2_hypoxia",
    ...
    (
        "dyn-stop-dyn",
        "Stop in the middle (dive-STA-dive)",
        "any",
        "main",
        "mental, co2",
...
mapping = [
    (
        ...
        "main",
        "sta-nwu-submax",
        "5 min strong HV (5:5); full recovery",
    ),
    (
        "STA",
        "STA 1",
        ...
        "main",
        "sta-co2-increasing",
        "start 2:00, +20-30s, 5xRB, until failure",
    ),
    (
        "STA",
        "STA 2",
        "Ex1 NWU with movement",
        "main",
        "sta-nwu-submax-moving",
        "5 min soft HV (4:6, belly only); full recovery",
    ),
    (
        "STA",
        "STA 2",
        "Ex2 long CO2 table V-shaped",
        "main",
        "sta-co2-vshape",
        "4 min soft HV, start ~50% PB, 2xRB",
    ),
    (
        "STA",
        "STA 3",
        ...
        "main",
        "sta-el",
        "5x relaxed belly + 5x pull-and-release, 2-3 min recovery",
    ),
    (
        "STA",
        "STA 4",
        ...
        "main",
        "sta-1c-plus",
        "2-4 min prep; 1C +0/20/40/60/80s; alt fixed 2-min recovery",
    ),
    (
        "STA",
        "STA 5",
        ...
        "main",
        "sta-frc-awareness",
        "FL->FRC, 15/30/45s, 1-min recovery",
    ),
    (
        "STA",
        "STA 6",
        ...
        "main",
        "sta-co2-1breath",
        "20-60s FL, 1xRB, moderate urge throughout",
    ),
    (
        "STA",
        "STA 7",
    ...
    (
        ...
        "warmup",
        "sta-wu-frc-progression",
        "1-3 FRC holds before the volume work",
    ),
    (
        "STA",
        "STA 9",
        ...
        "main",
        "sta-el-fl-switch",
        "2 min 4:6 HV recovery; EL +20s / FL +20-30s",
    ),
    (
        "STA",
        "STA 10",
        "high-volume CO2 table",
        "main",
        "sta-high-volume",
        "reduced-prep variant ON; 70% PB x3",
    ),
    (
        "STA",
        "STA 10",
    ...
    (
        ...
        "main",
        "sta-progressive-fl",
        "5 min 5:5 HV, 1 FL at 70-80% PB (single-hold case)",
    ),
    (
        "STA",
        "STA 12",
        "breathing-cycle 1C ladder",
        "main",
        "sta-1c-plus",
        "cycles 10/15/20/25/30; 1C +0/20/40/60 then submax/max",
    ),
    (
        "STA",
        "STA 13",
    ...
    (
        ...
        "main",
        "sta-get-high",
        "2 min recovery only, +1 min each, until failure",
    ),
    (
        "STA",
        "STA 15",
        ...
        "main",
        "sta-co2-short-intense",
        "5-6 FL, 2-breath recovery (e.g. 6x2:30)",
    ),
    (
        "STA",
        "STA 15",
        ...
        "warmup",
        "sta-wu-frc-progression",
        "2 easy FRC to 4-5 contractions, full recovery",
    ),
    (
        "STA",
        "STA 16",
        ...
        "main",
        "sta-co2-decreasing-rec",
        "hold sized to contraction duration; recovery 60s->10s",
    ),
    (
        "STA",
        "STA 16",
    ...
    (
        ...
        "main",
        "sta-get-high",
        "2:2 HV, +30s each, target 90s-1min below PB",
    ),
    (
        "STA",
        "STA 19",
        "challenging second hold",
        "main",
        "sta-co2-second-hold",
        "70% PB, 3 RB, max second hold",
    ),
    (
        "STA",
        "STA 20",
        ...
        "warmup",
        "sta-wu-frc-progression",
        "3 FRC progressive + 2 FL 1C+30-60",
    ),
    (
        "STA",
        "STA 20",
        ...
        "main",
        "sta-progressive-fl",
        "comfortable -> strong sub-max -> max for the day",
    ),
    (
        "STA",
        "STA 21",
        ...
        "main",
        "sta-co2-1breath",
        "1C, 1C+10, 1C+20... +10s/rep, until quality drops",
    ),
    (
        "STA",
        "STA WU1",
    ...
    (
        ...
        "warmup",
        "sta-wu-contraction-delay",
        "repeat to 1C then max once timed right",
    ),
    (
        "STA",
        "STA WU4",
    ...
    (
        ...
        "warmup",
        "sta-wu-nwu-hard-start",
        "hard FL to PB-1min, recover, then max",
    ),
    (
        "STA",
        "STA WU6",
    ...
    (
        ...
        "warmup",
        "sta-el-fl-switch",
        "EL/FL below PB margins (same exercise as STA 9, used as warm-up)",
    ),
    # ----- DYN -----
    (
        "DYN",
        ...
        "main",
        "dyn-max-simulator",
        "80% PB, 2xRB, min{60m,30%PB}, 3xRB, max",
    ),
    (
        "DYN",
        "DYN 2",
        ...
        "main",
        "dyn-frc-sprint",
        "50-100m FRC, 4xRB, 3x25m sprints 10/20s rec, x3 sets",
    ),
    (
        "DYN",
        "DYN 3",
        "elastic sprint into DNF max",
        "main",
        "dyn-elastic-sprint-max",
        "4x15s elastic, 15s rest, max DNF; 2 sets",
    ),
    (
        "DYN",
        "DYN 4",
        ...
        "main",
        "dyn-stop-start",
        "2-3x STA to 1C/UTB then FL/crawl/FRC, further each dive (was a transition-ladder exercise; folded into stop-start)",
    ),
    (
        "DYN",
        "DYN 5",
        ...
        "main",
        "dyn-volume-maximize",
        "3 x long, 3 min recovery, MAXIMIZE total distance",
    ),
    (
        "DYN",
        "DYN 5",
        ...
        "main",
        "dyn-volume-technique",
        "8-10x {50m legs-only, 50m normal}; DNF",
    ),
    (
        "DYN",
        "DYN 6",
        ...
        "main",
        "dyn-descending",
        "long, -20%, -20% again, minimal recovery",
    ),
    ...
    (
        ...
        "main",
        "dyn-pyramid",
        "25-50-75-50-25, ~20 min worth of pyramids, 5 min rest, then another set",
    ),
    (
        "DYN",
        "DYN 8",
        "Ex1 stop-start crawl",
        "main",
        "dyn-stop-start",
        "STA then slow dynamic crawl",
    ),
    (
        "DYN",
        "DYN 8",
        ...
        "main",
        "dyn-volume-maximize",
        "4x/6x/8x any discipline; recovery 2min/1min/30s; MAXIMIZE total distance",
    ),
    ...
    (
        ...
        "main",
        "dyn-technique",
        "4x arms / 4x legs / 4x normal, count strokes (DNF)",
    ),
    (
        "DYN",
        "DYN 9",
    ...
    (
        ...
        "main",
        "dyn-inverse-pyramid",
        "2x {75-50-25-50-75} DYNb, recovery < swim time, cap 90s",
    ),
    ...
    (
        ...
        "main",
        "dyn-volume-fixed",
        "2x75m DNF, 5 min recovery (mental dive-plan focus)",
    ),
    (
        "DYN",
        "DYN 11",
    ...
    (
        ...
        "main",
        "dyn-volume-fixed",
        "6x75m DYNb/DYN, 90-120s recovery (turns/technique focus)",
    ),
    ...
    (
        "DYN",
        "DYN 13",
        "Ex2 inverse pyramid DNF",
        "main",
        "dyn-inverse-pyramid",
        "2x {75-50-25-50-75}, full recovery between sets",
    ),
    (
        "DYN",
        "DYN 14",
        ...
        "main",
        "dyn-stop-start",
        "1:30 STA then DNF sub-max (library labeled 'start-stop'; structurally STA->dive)",
    ),
    (
        "DYN",
        "DYN 14",
    ...
    (
        ...
        "cooldown",
        "dyn-hypercapnic",
        "500m hypercapnic swim (listed, no params)",
    ),
    (
        "DYN",
        "DYN 15",
        ...
        "main",
        "dyn-nwu-submax",
        "variant=to-1C: real discomfort + 3 contractions",
    ),
    ...
    (
        ...
        "main",
        "dyn-stop-start",
        "2x {1 min STA + 50m+ DNF}, full recovery",
    ),
    (
        "DYN",
        "DYN 16",
        ...
        "main",
        "dyn-volume-fixed",
        "6x50m DNF, 75-90s recovery",
    ),
    (
        "DYN",
        "DYN 17",
        "Ex1 broken 200m DYNb",
        "main",
        "dyn-broken-200",
        "75/50/3x25, minimum recovery, consistent pace",
    ),
    (
        "DYN",
        "DYN 17",
        ...
        "main",
        "dyn-sprints",
        "4x50m max sprint, max 2 min recovery",
    ),
    (
        "DYN",
        "DYN 18",
        ...
        "main",
        "dyn-frc-sprint",
        "2x {50m FRC, 4xRB, 6x25m sprints, 15s recovery}",
    ),
    (
        "DYN",
        "DYN 18",
        ...
        "main",
        "dyn-volume-fixed",
        "6x75m, recovery < 2 min, fast first 25m, sprint last 50m",
    ),
    (
        "DYN",
        "Cool-down",
        ...
        "cooldown",
        "dyn-hypercapnic",
        "300-500m, breathe every 7+ strokes, no stopping",
    ),
]
...
ws.append(
    [
        ...
        "Capacity focus",
        "Fixed structure",
        "Session params (-> editable defaults)",
    ]
)
for section, session, ex, role, cid, params in mapping:
    c = cat_by_id[cid]
...
REPS = {
    ...
    "dyn-max-simulator": [
        {
            "shape": "simple",
            "distance_target": {"unit": "pct_pb", "value": 80},
            "recovery": _BREATHS(2),
            "note": "80% PB dive",
        },
        {
            "shape": "simple",
            "distance_target": {"unit": "absolute", "value": 60},
            "recovery": _BREATHS(3),
            "note": "short leg (min{60m, 30% PB})",
        },
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "max"},
            "note": "max push",
        },
    ],
    ...
    "dyn-broken-200": [
        {
            "shape": "simple",
            "distance_target": {"unit": "absolute", "value": 75},
            "recovery": _MINIMAL,
        },
        {
            "shape": "simple",
        ...
        {
            "shape": "simple",
            "distance_target": {"unit": "absolute", "value": 25},
            "recovery": _MINIMAL,
        },
        {
            "shape": "simple",
    ...
    "sta-co2-short-intense": [
        {
            "shape": "simple",
            "hold_target": {"unit": "absolute", "value": 150},
            "recovery": _BREATHS(2),
        },
    ],
    ...
    "sta-co2-vshape": [
        {
            "shape": "simple",
            "hold_target": {"unit": "absolute", "value": s},
            "recovery": _BREATHS(2),
        }
    ...
    "sta-co2-decreasing-rec": [
        ...
        for r in (60, 50, 40, 30, 20, 15, 10)
    ],
    # STA 4 / STA 12: 1C+X ladder, +20s per rep, fixed ~2 min recovery.
    "sta-1c-plus": [
        {
            "shape": "simple",
            "hold_target": {"unit": "contraction_relative", "value": x},
            "recovery": _SECS(120),
        }
    ...
    "sta-frc-awareness": [
        ...
        for rep in (
            {
                "shape": "simple",
                "hold_target": {"unit": "absolute", "value": 60},
                "lung_volume": "FL",
                "recovery": _SECS(0),
                "note": "exhale to FRC, no breath",
            },
            {
                "shape": "simple",
                "hold_target": {"unit": "absolute", "value": s},
    ...
    "dyn-descending": [
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "long (doable)"},
            "recovery": _MINIMAL,
        },
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "~20% less"},
            "recovery": _MINIMAL,
        },
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "~20% less again"},
        },
    ],
    ...
    "dyn-pyramid": [
        {
            "shape": "simple",
            "distance_target": {"unit": "absolute", "value": d},
            "recovery": _MINIMAL,
        }
    ...
    "sta-nwu-submax": [
        {
            "shape": "simple",
            "prep_breathing": {"pattern": "5:5", "duration_s": 300},
            "hold_target": {"unit": "qualitative", "value": "submax"},
        },
    ],
    ...
    "sta-nwu-submax-moving": [
        {
            "shape": "simple",
            "prep_breathing": {"pattern": "4:6", "duration_s": 300},
            "hold_target": {"unit": "qualitative", "value": "submax"},
    ...
    "sta-co2-increasing": [
        ...
        for s in (120, 140, 160, 180, 200, 220)
    ],
    ...
    "sta-co2-second-hold": [
        {
            "shape": "simple",
            "prep_breathing": {"pattern": "5:5", "duration_s": 300},
            "hold_target": {"unit": "pct_pb", "value": 70},
            "recovery": _BREATHS(3),
        },
        {
            "shape": "simple",
    ...
    "sta-get-high": [
        {
            "shape": "simple",
            "prep_breathing": {"pattern": "2:2", "duration_s": 60},
            "hold_target": {"unit": "absolute", "value": s},
            "recovery": _SECS(60),
        }
    ...
    "sta-wu-frc-progression": [
        {
            ...
            "recovery": _SECS(210),
            **(
                {"note": "through the first hard contractions, not to hypoxia"}
                if s == 90
                else {}
            ),
        }
    ...
    "sta-wu-contraction-delay": [
        {
            "shape": "simple",
            "hold_target": {"unit": "contraction_relative", "value": 0},
            "recovery": _FULL,
            "note": "repeat until 1C arrives at the right time",
        },
        {
            "shape": "simple",
            "hold_target": {"unit": "contraction_relative", "value": 0},
    ...
    "sta-wu-nwu-hard-start": [
        {
            ...
            "recovery": _SECS(300),
            "note": "~PB-1 min; strong HV during recovery",
        },
        {"shape": "simple", "hold_target": {"unit": "qualitative", "value": "max"}},
    ],
    ...
    "dyn-nwu-submax": [
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "70-80% feel"},
    ...
    "dyn-volume-maximize": [
        {
            "shape": "simple",
            "distance_target": {"unit": "qualitative", "value": "maximize"},
            "recovery": _SECS(120),
            **(
                {"note": "goal: maximize total distance across the dives"}
                if i == 0
                else {}
            ),
        }
    ...
    "dyn-sprints": [
        {
            ...
            "distance_target": {"unit": "absolute", "value": 50},
            "pace": "max_sprint",
            "recovery": {"type": "cap", "value": 120, "unit": "time"},
        }
        for _ in range(2)
    ],
    ...
    "dyn-stop-start": [
        {
            "shape": "stop-start",
            "hold_target": {"unit": "absolute", "value": 90},
            "distance_target": {"unit": "absolute", "value": 50},
            "recovery": _FULL,
    ...
    "dyn-start-stop": [
        {
            "shape": "start-stop",
            "distance_target": {"unit": "absolute", "value": 60},
            "hold_target": {"unit": "absolute", "value": 15},
            "recovery": _FULL,
...
_PCT = lambda p, **kw: {
    "shape": "simple",
    "distance_target": {"unit": "pct_pb", "value": p},
    **kw,
}
_SWIM = lambda m, note=None: {
    "shape": "continuous-protocol",
    "distance_target": {"unit": "absolute", "value": m},
    ...
    for i in range(n):
        rows.append(
            _D(
                25,
                pace="max_sprint",
                note="surface dolphin kick, snorkel or on your back",
            )
        )
        rows.append(
            _D(
                ...
                pace="max_sprint",
                recovery=_CAP(rest),
                note="duck dive, underwater sprint",
            )
        )
    return rows
...
def _ladder(n75, n50):
    return [_D(75, recovery=_LE_SWIM) for _ in range(n75)] + [
        _D(50, recovery=_LE_SWIM) for _ in range(n50)
    ]


_RELAXED_100 = lambda: [_D(100, pace="relaxed", note="any style, breathe normally")]
_OVER_UNDER = lambda: [
    _D(25, note="freestyle on the surface"),
    _D(25, note="turn, DYNb underwater, no stop"),
]
_TORTUGA = lambda: [
    _D(50, pace="relaxed", note="longest possible time; distance irrelevant")
]
...
SEALS_PHASES = {
    ...
    "seals-pyramid": {
        "base": {
            "reps": [
                _D(50, recovery=_CAP(60)),
                _D(25, recovery=_CAP(60)),
                _D(25, recovery=_CAP(60)),
                _D(50),
            ]
        },
    ...
    "seals-recovery-swim": {
        p: {"reps": _RELAXED_100()}
        for p in ("base", "build", "specialization", "taper")
    },
    "seals-over-under": {
        p: {"reps": _OVER_UNDER(), "set_repeat": 2} for p in ("base", "build")
    },
    "seals-sprints": {
        "base": {
            "reps": [_D(25, pace="max_sprint", recovery=_SECS(20)) for _ in range(6)],
            "set_repeat": 2,
            "recovery_inter": _SECS(120),
        },
        "build": {
            "reps": [_D(75, pace="max_sprint", recovery=_CAP(120)) for _ in range(5)],
            "set_repeat": 1,
        },
    },
    "seals-dolphin-sprints": {
    ...
    "seals-submax": {
        ...
        "specialization": {
            "reps": [_PCT(70, note="70-75%; no upper limit; surface before your limit")]
        },
    ...
    seals_templates.append(
        {
            ...
            **(
                {"recovery_inter": d0["recovery_inter"]}
                if "recovery_inter" in d0
                else {}
            ),
